"""Stage, promote, and verify a Structural Fab Sales snapshot in Supabase."""
...
def publish(snapshot_path: Path, credential_path: Path) -> dict[str, Any]:
    ...
    metadata = rpc(base, key, credentials["operator_verification_key"], "sfab_sales_snapshot_metadata", {})
    current = metadata[0] if isinstance(metadata, list) and metadata else {}
    if current.get("source_sha256") == snapshot["sha256"]:
        touched = rpc(base, key, credentials["current_ar_promotion_key"], "sfab_sales_heartbeat_snapshot", {"p_snapshot_id": current["snapshot_id"]})
        metadata = rpc(base, key, credentials["operator_verification_key"], "sfab_sales_snapshot_metadata", {})
    ...
    snapshot_id = rpc(base, key, credentials["current_ar_ingestion_key"], "sfab_sales_stage_snapshot", {
        "p_source_sha256": snapshot["sha256"], "p_as_of": snapshot["as_of"],
        "p_refreshed_at": snapshot["refreshed_at"], "p_payload": snapshot,
    })
    rpc(base, key, credentials["current_ar_promotion_key"], "sfab_sales_promote_snapshot", {"p_snapshot_id": snapshot_id})
    metadata = rpc(base, key, credentials["operator_verification_key"], "sfab_sales_snapshot_metadata", {})
    row = metadata[0] if isinstance(metadata, list) and metadata else {}
